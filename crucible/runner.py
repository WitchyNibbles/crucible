...
MAX_AUDIT_ROUNDS = 3
ARTIFACT_HEADER = "# Generated by crucible\n\n"
...
class AgentResult:
    ...
    @property
    def real(self) -> bool:
        return bool(self.metadata.get("real", False))
...
class ArtifactWriter:
    ...
    def write(
        ...
        *,
        ...
        stripped = textwrap.dedent(content.lstrip("\n"))
        if add_header and not stripped.startswith(ARTIFACT_HEADER_PREFIX):
            body = ARTIFACT_HEADER + stripped
        else:
            body = stripped
...
class DefaultSubagentWorker(AgentBridge):
    ...
    * `typescript` frontend stub: `app.ts`, `README.md`
    ...
    ALLOWED_MODELS = {"default", "stub", "llm"}
    ARTIFACT_HEADER_PREFIX = "# Generated by crucible"
    REQUIRE_SUPPORTED_LANGUAGE = "Unsupported language for DefaultSubagentWorker"
    REQUIRE_SUPPORTED_MODEL = "Unsupported model for DefaultSubagentWorker"

    TIER_ARTIFACTS: dict[str, dict[str, str]] = {
        "python": {
            "app.py": textwrap.dedent("""
                def main(*, payload: str = "hello from DefaultSubagentWorker") -> str:
                    payload = (payload or "").strip() or "hello from DefaultSubagentWorker"
                    print(payload)
                    return payload
                ...
                if __name__ == "__main__":
                    value = main()
                    assert value, "main() should return a non-empty payload"
            """).lstrip("\n"),
        ...
        "typescript": {
            "app.ts": textwrap.dedent("""
                export function main(*, payload = "hello from DefaultSubagentWorker"): string {
                    const value = String(payload ?? "hello from DefaultSubagentWorker").trim() || "hello from DefaultSubagentWorker";
                    console.log(value);
                    return value;
                }
                ...
                if (require.main === module) {
                    const value = main();
                    if (!value) throw new Error("main() should return a non-empty payload");
                }
            """).lstrip("\n"),
            "README.md": textwrap.dedent("""
                # Generated by crucible
    ...
    def run_language_task(
        ...
        model = (context.get("model") or "default").strip().lower()
        if model not in self.ALLOWED_MODELS:
            raise ValueError(
                f"{self.REQUIRE_SUPPORTED_MODEL}: {model!r}"
            )
